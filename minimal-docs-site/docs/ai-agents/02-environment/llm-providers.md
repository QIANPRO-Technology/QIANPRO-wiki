...
本課程預設使用 **OpenAI 相容 API**,任何能提供該介面的後端都能替換 — 包含自架的 vLLM。

## 主流選擇

| 供應商 | 代表模型 | 強項 | 備註 |
|--------|----------|------|------|
| **OpenAI** | gpt-4o, gpt-4o-mini, o1 | 工具呼叫穩定、生態完整 | 課程預設 |
...
| **自架 vLLM** | Llama 3.1/3.3, Qwen 2.5, DeepSeek | 資料不出門、可客製 | 下一節 |
...
| 本地 Llama 3.3 70B | 硬體攤提 | 硬體攤提 | 資料敏感、大量推論 |

## 教學建議

| 課程階段 | 用什麼 |
|---------|--------|
| Ch 01-04(概念 + 工具) | `gpt-4o-mini`(便宜快) |
| Ch 05-08(Graph + 多 Agent) | `gpt-4o`(工具呼叫穩定) |
| Ch 09-10(RAG + 部署) | 本地 vLLM(示範地端方案) |

## 延伸

- 下一節:[在 DGX Spark 上架 vLLM](./vllm-dgx-spark)
